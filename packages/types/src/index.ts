...
// ─────────────────────────────────────────────────────────────
// Tool Tab Plugin
// 도구 관리 페이지에 탭으로 끼워지는 플러그인 인터페이스
// ─────────────────────────────────────────────────────────────
export interface ToolTabPlugin {
  /** 플러그인 고유 ID (탭 key로도 사용) */
  id: string;
  /** 플러그인 이름 */
  name: string;
  ...
  order: number;
  /** 탭 컨텐츠 컴포넌트 */
  component: ComponentType<ToolTabPluginProps>;
}

export interface ToolTabPluginProps {
  /** 다른 섹션으로 이동 */
  onNavigate?: (sectionId: string) => void;
  /** 서브 툴바 콘텐츠를 상위로 전달 */
  onSubToolbarChange?: (content: React.ReactNode) => void;
...
// ─────────────────────────────────────────────────────────────
// Prompt Tab Plugin
// 프롬프트 관리 페이지에 탭으로 끼워지는 플러그인 인터페이스
// ─────────────────────────────────────────────────────────────
export interface PromptTabPlugin {
  /** 플러그인 고유 ID (탭 key로도 사용) */
  id: string;
  /** 플러그인 이름 */
  name: string;
  ...
  order: number;
  /** 탭 컨텐츠 컴포넌트 */
  component: ComponentType<PromptTabPluginProps>;
}

export interface PromptTabPluginProps {
  /** 다른 섹션으로 이동 */
  onNavigate?: (sectionId: string) => void;
  /** 서브 툴바 콘텐츠를 상위로 전달 */
  onSubToolbarChange?: (content: React.ReactNode) => void;
...
class FeatureRegistryClass {
  ...
  // ── ToolTabPlugin ──
  registerToolTabPlugin(plugin: ToolTabPlugin): void {
    this.toolTabPlugins.set(plugin.id, plugin);
  }

  getToolTabPlugins(): ToolTabPlugin[] {
    return Array.from(this.toolTabPlugins.values())
      .sort((a, b) => a.order - b.order);
  }
  ...
  // ── PromptTabPlugin ──
  registerPromptTabPlugin(plugin: PromptTabPlugin): void {
    this.promptTabPlugins.set(plugin.id, plugin);
  }
