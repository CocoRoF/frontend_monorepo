...
import { isMultiType, getPrimaryType, formatTypeDisplay } from '../../../utils/canvas-utils';
...
export const getLocalizedNodeName = (
    nodeName: string,
    nodeNameKo?: string,
    locale: string = 'en'
): string => {
    return (locale === 'ko' && nodeNameKo) ? nodeNameKo : nodeName;
};
...
export const getPortTypeDisplay = (typeString?: string): string => {
    if (!typeString) return '';
    return formatTypeDisplay(typeString);
};
