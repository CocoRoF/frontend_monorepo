...
import { FiPlus } from '@xgen/icons';
import { useTranslation } from '@xgen/i18n';
import { createPrompt, uploadToPromptStore } from './api';
...
export const PromptCreateModal: React.FC<PromptCreateModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const response = await createPrompt(form) as { data?: { success?: boolean; prompt?: { id: number } } };
      if (form.public_available && response?.data?.prompt?.id) {
        try {
          await uploadToPromptStore(response.data.prompt.id);
        } catch {
          // Store upload is optional
        }
      }
      toast.success(t('promptManagementStorage.modal.createSuccess'));
      onSuccess();
      onClose();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : t('promptManagementStorage.modal.createFailed'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={t('promptManagementStorage.modal.createTitle')}
      size="md"
      footer={
        <div className="flex justify-end gap-2">
          ...
          >
            ...
            {submitting ? t('promptManagementStorage.modal.creating') : t('promptManagementStorage.modal.create')}
          </Button>
        </div>
      }
    >
